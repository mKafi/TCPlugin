...
class PayoffData extends WP_List_Table {
        ...
        public function table_data() {
			...
			foreach($posts_array AS $k=>$v){
				...
				$sales_goal = 10;
				if(!empty($p_meta['sales_goal'][0])){
					$sales_goal = $p_meta['sales_goal'][0];
				}
				
				$total_sales = 0;
				if(!empty($p_meta['total_sales'][0])){
					$total_sales = $p_meta['total_sales'][0];
				}
...
if((isset($_GET['uid']) && is_numeric($_GET['uid']) && $_GET['uid'] > 0) && (isset($_GET['amount']) && is_numeric($_GET['amount']) && $_GET['amount'] > 0)){
	...
	<form action="" method="post" class="paybtn-cont">
		<div class="pay-notice">The amount <span>$<?php echo $_GET['amount']; ?></span> will send to this email <span title="<?php echo $user_info->first_name.' '.$user_info->last_name; ?>"><?php echo $user_info->user_email; ?><span></div>
